...
FINAL_STATE_DIR = "final_states/mapf_team_rew/"
...
# Team reward constants
TEAM_STEP_PENALTY = -0.1  # Per time step until completion
TEAM_COMPLETION_BONUS = 50.0  # When all agents reach goals

class TeamRewardTracker:
    def __init__(self, n_agents):
        self.n_agents = n_agents
        self.team_reward = 0.0
        self.completion_bonus_given = False

    def update(self, completed_agents_count):
        """Update team reward based on current state"""
        # Apply time step penalty only if not completed
        if not self.completion_bonus_given:
            self.team_reward += TEAM_STEP_PENALTY

        # Check for full team completion
        all_agents_at_goals = (completed_agents_count == self.n_agents)

        if all_agents_at_goals and not self.completion_bonus_given:
            self.team_reward += TEAM_COMPLETION_BONUS
            self.completion_bonus_given = True
            return True  # Indicate team completion
        
        return False  # No team completion this step
...
def save_final_state(env, accumulated_rewards, team_reward, step, reason):
    ...
    with open(state_file, 'w') as f:
        ...
        f.write(f"Team Reward: {team_reward:.2f}\n")  # Add team reward to output
        f.write("-" * 20 + "\n")
...
def main():
    ...
    initial_goals = goals[:N_AGENTS] # Store initial goals

    # Initialize team reward tracker
    team_tracker = TeamRewardTracker(N_AGENTS)
    ...
    try:
        ...
        env.unwrapped.goals = list(initial_goals) # Set initial goals in env

        obs, info = env.reset()
        accumulated_rewards = [0.0] * N_AGENTS
        completed_agents = set() # Track agents that reached their initial goal
        step = 0
        running = True
        ...
        # Add team reward tracking
        env.unwrapped._team_reward = 0.0
        ...
        while running:
            ...
            actions = []
            for i in range(N_AGENTS):
                # Check if agent already completed its initial goal
                if i in completed_agents:
                    action = ACTION_NOOP # Stay put if goal reached
                else:
                    action = select_next_move(
                        i,
                        current_agent_positions[i],
                        current_orientations[i],
                        initial_goals[i],  # Use the initial, stationary goal
                        obstacles,
            ...
            # --- Environment Step ---
            # IMPORTANT: The base environment's step might still relocate goals
            # We ignore the environment's dynamic goals after the step
            obs_next, rewards, terminated, truncated, info = env.step([a for a in final_actions])
            # Reset env goals to initial goals to prevent dynamic relocation effect
            env.unwrapped.goals = list(initial_goals) 
            
            # --- Reward Calculation & State Update ---
            current_rewards = [0.0] * N_AGENTS
            newly_completed = set()
            for i in range(N_AGENTS):
                # Only accumulate reward if agent hasn't completed its goal yet
                if i not in completed_agents:
                    current_rewards[i] = rewards[i]
                    accumulated_rewards[i] += rewards[i]
                    # Check if this agent just reached its initial goal
                    if current_agent_positions[i] == initial_goals[i]:
                        completed_agents.add(i)
                        newly_completed.add(i)
                        print(f"Agent {i+1} reached initial goal {initial_goals[i]}!")

            # Update team reward based on the count of completed agents
            team_completion = team_tracker.update(len(completed_agents))
            env.unwrapped._team_reward = team_tracker.get_reward()
            ...
            if step >= MAX_STEPS:
                ...
                running = False
            elif team_completion:
                print("Team completed all initial goals!")
                running = False

        # --- End of Simulation ---
        reason = "Team completion" if team_completion else (
            "Max steps reached" if step >= MAX_STEPS else (
            "ESC pressed" # Only remaining possibility if not completed/max_steps
        ))
        save_final_state(env, accumulated_rewards, team_tracker.get_reward(), step, reason)

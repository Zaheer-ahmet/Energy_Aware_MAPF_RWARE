"""
Automated BFS play and evaluation script for Comb_warehouse.py

Features:
- Configurable number of agents (via --n_agents) and max battery life (via --battery_max)
- 2+ agents, 2+ goals, 1+ charging stations (green tile)
...
- Battery logic: agents lose 1 battery per move, recharge to max (battery_max) on charging station
...
class DesiredAlgorithmBFSEnv:
    def __init__(self, layout_path=None, n_agents=None, battery_max=10, **kwargs):
        if not layout_path:
        ...
        with open(layout_path, 'r') as f:
            layout_str = f.read()
        grid, goals, charging_stations, grid_size = parse_layout_file(layout_path)
        ...
        self.battery_max = battery_max  # Max battery life variable (move before env creation)
        self.env = HRPFTeamWarehouse(
            ...
            render_mode="human",
            layout=layout_str,
            battery_max=self.battery_max  # Pass battery_max to environment
        )
        self.env.charging_stations = self.charging_stations
        self.running = True
        self.t = 0
        ...
        self.agents_going_to_charge = [False] * self.n_agents  # Track agents heading to charge
    ...
    def _setup_report_file(self):
        report_dir = os.path.join("Test_Results_Logs", "BFS_Charge_Play")
    ...
    def _get_nearest_charging_station(self, agent_pos):
        min_dist = float('inf')
        nearest_station = None
        stations = self.charging_stations if self.using_custom_layout else self.env.charging_stations
        for station in stations:
            sx, sy = get_station_xy(station)
            dist = abs(agent_pos[0] - sx) + abs(agent_pos[1] - sy)
            if dist < min_dist:
                min_dist = dist
                nearest_station = (sx, sy)
    ...
    def _cycle(self):
        ...
        while running:
            ...
            for i in range(self.n_agents):
                ...
                self.agents_going_to_charge[i] = False
            ...
            self.agents_going_to_charge = [False] * self.n_agents
            ...
            while not episode_done and running:
                ...
                batteries_before_step = list(self.agent_batteries) # Store battery before the step
                ...
                for i in range(self.n_agents):
                    ...
                    if self.agent_batteries[i] == 0 and not self.agents_going_to_charge[i] and (agent.x, agent.y) != self.original_goals[i]:
                        # Find nearest available charging station (not occupied by another agent)
                        occupied_stations = set((a.x, a.y) for a in self.env.unwrapped.agents)
                        available_stations = [s for s in self.charging_stations if s not in occupied_stations]
                        charging_station_pos = None
                        if available_stations:
                            # Pick the nearest available
                            charging_station_pos = min(available_stations, key=lambda s: abs(current_agent_pos[0]-s[0])+abs(current_agent_pos[1]-s[1]))
                        else:
                            # All stations occupied, NOOP for this step
                            agent_paths[i] = []
                            self.env.goals[i] = current_agent_pos
                            continue
                        goal_for_path_planning = charging_station_pos
                        self.agents_going_to_charge[i] = True
                    elif self.agents_going_to_charge[i]:
                        if self._is_on_charging_station(agent) and self.agent_batteries[i] == self.battery_max:
                            self.agents_going_to_charge[i] = False
                            # This is now tracked by comparing battery levels
                            # recharges[i] += 1 
                            # Always restore goal to original after charging, even if already at goal
                            goal_for_path_planning = self.original_goals[i]
                            self.env.goals[i] = self.original_goals[i]  # Explicitly restore in env
                        else:
                            # Same logic: only target available stations
                            occupied_stations = set((a.x, a.y) for a in self.env.unwrapped.agents)
                            available_stations = [s for s in self.charging_stations if s not in occupied_stations]
                            charging_station_pos = None
                            if available_stations:
                    ...
                    if current_agent_pos == self.original_goals[i] and not self.agents_going_to_charge[i]:
                        agent_paths[i] = []
                        continue
                ...
                self.agent_batteries = [agent.battery for agent in self.env.unwrapped.agents]

                # New recharge detection logic: count any time battery increases
                for i in range(self.n_agents):
                    if self.agent_batteries[i] > batteries_before_step[i]:
                        recharges[i] += 1
                ...
                if done:
                    # The following prints are commented out to keep the output file clean.
...
if __name__ == "__main__":
    ...
    parser.add_argument('--battery_max', type=int, default=10, help='Maximum battery life for each agent (default: 10)')
    ...
    game = DesiredAlgorithmBFSEnv(layout_path=args.layout, n_agents=args.n_agents, battery_max=args.battery_max, max_episodes=args.max_episodes, run_id=args.run_id) 